...
def Lance_All_Analyse(filename, out_path, correct_methodo):
    ...
    if correct_methodo == "ICA":
        ...
        # Regression
        # raw_corr_reg = Coma_Analysis_Def.Correction_Regression(raw, picks)
    ...
    if correct_methodo != "ASR":
        ...
        epoch_corr = mne.Epochs(raw_corr.filter(0, 30), events, **cfg, reject = def_th_reject)
    
    else:
        
        def_th_reject = dict(eeg = 1)
        rawCalibAsr = raw.copy().drop_channels(raw.info['bads'])
        epoch_corr = Coma_Analysis_Def.Correction_ASR(rawCalibAsr,
                                                      mne.Epochs(raw_corr.filter(0, 30), events, **cfg, reject = def_th_reject),
                                                      picks,
                                                      events,
                                                      event_id)
        del rawCalibAsr
    ...
    Coma_Analysis_Def.Disp_stat_clust_all(epoch_corr,
                                          'Std/Dist',
                                          'Dev/Dist',
                                          'cond: Distractive    cor: ICA').savefig(out_path + prefix_out_filename + '_Distractive_STD_DEV.pdf')
    
    Coma_Analysis_Def.Disp_stat_clust_all(epoch_corr,
                                          'Std/Act',
                                          'Dev/Act',
                                          'cond: Active    cor: ICA').savefig(out_path + prefix_out_filename + '_Active_STD_DEV.pdf')
    
    Coma_Analysis_Def.Disp_stat_clust_all(epoch_corr,
                                          'Dev/Dist',
                                          'Dev/Act',
                                          'cond: Active/Dist    cor: ICA').savefig(out_path + prefix_out_filename + '_ActDis_DEV.pdf')
    
    Coma_Analysis_Def.Disp_stat_clust_all(epoch_corr,
                                          'Std/Dist',
                                          'Std/Act',
                                          'cond: Active/Dist    cor: ICA').savefig(out_path + prefix_out_filename + '_ActDis_STD.pdf')
...
## ERP standards
    ...
    if correct_methodo != "ASR":    
        ...
        mne.Epochs(raw_corr.filter(2, 30), event_std, **cfg, reject = def_th_reject).average().plot(titles = 'All Stim').savefig(out_path + prefix_out_filename + '_All_Stim.pdf')
        plt.gca().set_ylim(plt.gca().get_ylim()[::-1])
    
    else:
    
        def_th_reject = dict(eeg = 1)
        rawCalibAsr = raw_corr.copy().drop_channels(raw_corr.info['bads'])
        epoch_corr = Coma_Analysis_Def.Correction_ASR(rawCalibAsr,
                                                      mne.Epochs(raw_corr.filter(2, 30), event_std, **cfg, reject = def_th_reject),
                                                      picks,
                                                      event_std,
                                                      cfg['event_id']).average().plot(titles = 'All Stim')
        del rawCalibAsr
...
## MMN    std VS dev    front average  -  mastoid average
    ...
    raw_tmp = raw_corr.copy()
    raw_tmp.info['bads'] = ['EMG1+', 'EMG2+','EMG3+','EMG4+',
                      'PNG1+','PNG2+','PNG3+','PNG4+',
                      'ECG1+',
                      'MKR+', 'STI 014',
                      'O2']
    ...
    front = array_raw[np.array([11, 12, 14, 15]),:].mean(0).reshape(1, array_raw.shape[1])
    masto = array_raw[np.array([16,17]),:].mean(0).reshape(1, array_raw.shape[1])
    PZ = array_raw[np.array([13]),:]
    sti014 = np.zeros((1,array_raw.shape[1]))
    eog = array_raw[np.array([9]),:]
    ...
    if correct_methodo != "ASR":    
        ...
        epoch_mean_ica = mne.Epochs(mean_raw_corr_ica.filter(2, 30), events, **cfg, reject = def_th_reject)
    
    else:
    
        def_th_reject = dict(eeg = 1)
        rawCalibAsr = mean_raw_corr_ica.copy().drop_channels(mean_raw_corr_ica.info['bads'])
        epoch_mean_ica = Coma_Analysis_Def.Correction_ASR(rawCalibAsr,
                                                      mne.Epochs(mean_raw_corr_ica.filter(2, 30), events, **cfg, reject = def_th_reject),
                                                      picks,
                                                      events,
                                                      cfg['event_id'])
        del rawCalibAsr
    ...
    Coma_Analysis_Def.Disp_stat_clust_all(epoch_mean_ica,
                                          'Std/Pass',
                                          'Dev/Pass',
                                          'cond: Passive    cor: ICA').savefig(out_path + prefix_out_filename + '_Passive_STD_DEV_4F2M.pdf')
    
    Coma_Analysis_Def.Disp_stat_clust_all(epoch_mean_ica,
                                          'Std/Dist',
                                          'Dev/Dist',
                                          'cond: Distractive    cor: ICA').savefig(out_path + prefix_out_filename + '_Distractive_STD_DEV_4F2M.pdf')

    Coma_Analysis_Def.Disp_stat_clust_all(epoch_mean_ica,
                                          'Std/Act',
                                          'Dev/Act',
                                          'cond: Active    cor: ICA').savefig(out_path + prefix_out_filename + '_Active_STD_DEV_4F2M.pdf')
    ...
    if correct_methodo != "ASR":    
        ...
        epoch_mean_ica = mne.Epochs(mean_raw_corr_ica.filter(0.1, 30), event_std, **cfg, reject = def_th_reject)
        ...
        rawCalibAsr = mean_raw_corr_ica.copy().drop_channels(mean_raw_corr_ica.info['bads'])
        epoch_mean_ica = Coma_Analysis_Def.Correction_ASR(rawCalibAsr,
                                                      mne.Epochs(mean_raw_corr_ica.filter(2, 30), event_std, **cfg, reject = def_th_reject),
                                                      picks,
                                                      event_std,
                                                      cfg['event_id'])
        del rawCalibAsr
    ...
    Coma_Analysis_Def.Disp_stat_clust_all(epoch_mean_ica,
                                          'Std',
                                          'Dev',
                                          'cond: ALL    cor: ICA').savefig(out_path + prefix_out_filename + '_ALL_STD_DEV_4F2M.pdf')
