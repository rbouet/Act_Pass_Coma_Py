...
MAJ
    04/05/21    artefact rejection methode
                        - amplitude threshold fixed
                        - rejection percentage fixed
...
def Lance_All_Analyse(filename, out_path, correct_methodo = 'None', rejection_ampl = 10e-5, rejection_pct = 10):

    """
    
    correct_methodo:    'Regress', 'ICA' or 'ASR'
    rejection_ampl:     10e-5
    rejection_pct:      20
...
#                        'M1', 'M2']
    ...
    quel_id = np.where((events[:,0] > 2500*raw.info['sfreq']) & ((events[:,2] == 21) | (events[:,2] == 22)))[0]
    events[quel_id,2] = 6693
    ...
    if correct_methodo != 'ASR':
        ...
        #ejection_ampl = dict(eeg = ejection_ampl)
        fig_rej, def_th_reject = Coma_Analysis_Def.Eval_rejection(raw_corr, events, cfg, dict(eeg = rejection_ampl), rejection_pct)
