...
def Lance_All_Analyse(filename, out_path, correct_methodo):
    ...
    if correct_methodo == "ICA":
        raw_corr_ica, ica, score_ica = Coma_Analysis_Def.Correction_ICA(raw, picks)
        ...
        raw_corr_ica = Coma_Analysis_Def.Correction_Regression(raw, picks)
    ...
    # mne.viz.plot_events(events, raw.info['sfreq'], raw.first_samp, color=color,
    #                    event_id=event_id).savefig('chemin')
    ...
    fig_rej, def_th_reject = Coma_Analysis_Def.Eval_rejection(raw_corr_ica, events, cfg, def_th_reject)
    
    fig_rej.savefig(out_path + prefix_out_filename + '_PCT_Rejection.pdf')
    del fig_rej
    
    # epoch_raw = mne.Epochs(raw.filter(0.1, 30), events, **cfg)
    # epoch_reject = mne.Epochs(raw.filter(0.1, 30), events, reject = dict(eeg = 6e-5), **cfg)
    # epoch_reg = mne.Epochs(raw_corr_reg.filter(0.1, 30), events, **cfg)
    epoch_ica = mne.Epochs(raw_corr_ica.filter(0, 30), events, **cfg, reject = def_th_reject)
        
    
    #______________________________________________________________________________
    # Stat
    
    Coma_Analysis_Def.Disp_stat_clust_all(epoch_ica,
                                          'Std/Dist',
                                          'Dev/Dist',
...
## ERP standards
    ...
    mne.Epochs(raw_corr_ica.filter(2, 30), event_std, **cfg).average().plot(titles = 'All Stim').savefig(out_path + prefix_out_filename + '_All_Stim.pdf')
    plt.gca().set_ylim(plt.gca().get_ylim()[::-1])

##______________________________________________________________________________
## MMN    std VS dev    front average  -  mastoid average

    raw_tmp = raw_corr_ica.copy()
    ...
    del array_raw, front, masto, sti014
